...
-- Steps
Insert into VBATCH.STEPS (ID,TYPE,SHORT_DESC,LONG_DESC,CLASS_PATH,PARAM1,PARAM2,PARAM3,EXTRACT_MAX_REC,EXTRACT_MAX_REC_PER_FILE,EXTRACT_COMMIT_FREQ,EXTRACT_SQL,OUTPUT_FILE_FORMAT,OUTPUT_FILENAME_PREFIX,OUTPUT_FILENAME_POSTFIX) values (1,'Extract','Replenishment','Extract replenishment.','com.vco.ExtractDBStep',null,null,null,15,null,1000,'select p.tran_type, p.tran_code, p.tran_nbr, p.seq_nbr, p.whse, p.sku_id, p.cntr_nbr, p.wave_nbr, p.pkt_ctrl_nbr, p.pkt_seq_nbr, p.create_date_time ok1 from prod_trkg_tran p /* where */  order by p.create_date_time',null,null,null);
