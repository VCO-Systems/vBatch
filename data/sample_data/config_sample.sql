...
Insert into VBATCH.JOB_DEFINITION (ID,ORDER_NUM,SHORT_DESC,LONG_DESC) values (100,2,'Replenishment (Test)','replenishment transactions');
Insert into VBATCH.JOB_DEFINITION (ID,ORDER_NUM,SHORT_DESC,LONG_DESC) values (101,3,'Putaway (Test)','Putaway transactions');
...
Insert into VBATCH.STEPS (ID,TYPE,SHORT_DESC,LONG_DESC,CLASS_PATH,EXTRACT_MAX_REC,EXTRACT_MAX_REC_PER_FILE,EXTRACT_COMMIT_FREQ,EXTRACT_SQL,OUTPUT_FILE_FORMAT,OUTPUT_FILENAME_PREFIX,OUTPUT_FILENAME_POSTFIX,PARAM1,PARAM2,PARAM3) values (4,'Extract','PUTAWAY_CS_OBTAIN','Putaway Case (Obtain Record)','com.vco.ExtractDBStep',250,null,1000,'select ptt.create_date_time as OK1, ptt.tran_nbr as PK1, ptt.seq_nbr as PK2, ''CWMS0104'', ''A'',ptt.seq_nbr, '''', ptt.tran_nbr, to_char(ptt.create_date_time, ''yyyymmdd'') plan_date, '''',ptt.plt_id, ''17532'', '''', '''', '''', ''O'', ''PUTAWAY CASE'', '''' , ''4443'', ptt.from_locn, ''1'', '''', '''', '''', '''', '''', '''','''', '''', '''', '''','''', '''', '''', '''','''', '''', '''', '''', '''', '''', ptt.user_id, to_char(ptt.begin_date, ''yyyymmddhh24miss'') sign_on_time, '''', '''', '''', '''', ''''  from prod_trkg_tran ptt, item_master im, locn_hdr lh  where ptt.create_date_time > to_date(''20140101'', ''YYYYMMDD'') and ptt.sku_id = im.sku_id  and ptt.from_locn = lh.locn_id  and ptt.menu_optn_name = ''WMT Dir Ptwy Cs'' AND ptt.create_date_time > to_date(''20140101'', ''YYYYMMDD'') /* where */ order by ptt.create_date_time asc;',null,null,null,null,null,null);
Insert into VBATCH.STEPS (ID,TYPE,SHORT_DESC,LONG_DESC,CLASS_PATH,EXTRACT_MAX_REC,EXTRACT_MAX_REC_PER_FILE,EXTRACT_COMMIT_FREQ,EXTRACT_SQL,OUTPUT_FILE_FORMAT,OUTPUT_FILENAME_PREFIX,OUTPUT_FILENAME_POSTFIX,PARAM1,PARAM2,PARAM3) values (5,'CSV','PUTAWAY_CS_OBTAIN','Putaway Case (Obtain Record)','com.vco.GenerateCSVStep',null,100,null,null,'csv','PTWY_CS_b{batch_num}_{dt}_W001_{seq}.csv',null,null,null,null);    

-- job_steps_xref
Insert into VBATCH.JOB_STEPS_XREF (ID,JOB_DEFINITION_ID,STEP_ID,JOB_STEP_SEQ,SPECIAL_MODE) values (1,100,1,1,null);
Insert into VBATCH.JOB_STEPS_XREF (ID,JOB_DEFINITION_ID,STEP_ID,JOB_STEP_SEQ,SPECIAL_MODE) values (2,100,2,2,null);

Insert into VBATCH.JOB_STEPS_XREF (ID,JOB_DEFINITION_ID,STEP_ID,JOB_STEP_SEQ,SPECIAL_MODE) values (3,1,3,1,null);
Insert into VBATCH.JOB_STEPS_XREF (ID,JOB_DEFINITION_ID,STEP_ID,JOB_STEP_SEQ,SPECIAL_MODE) values (4,1,4,2,null);
Insert into VBATCH.JOB_STEPS_XREF (ID,JOB_DEFINITION_ID,STEP_ID,JOB_STEP_SEQ,SPECIAL_MODE) values (5,100,3,3,null);
